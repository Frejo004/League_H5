-- ============================================================
-- Migration 014 — get_standings RPC
-- Computes the full standings table server-side, including
-- per-team form (last 5 results), using season point settings.
-- ============================================================
...
returns table (
  ...
  goal_diff   integer,
  points      integer,
  form        text   -- last 5 results as string e.g. 'W,D,L,W,W'
)
...
declare
  ...
  v_pts_loss  integer;
begin
  -- Load point values for this season (fallback to standard 3/1/0)
  select
  ...
  -- Expand each match into two team-result rows
  team_results as (
    select
      home_team_id                                    as tid,
      home_score                                      as gf,
  ...
  -- Compute form: last 5 results ordered chronologically
  team_form as (
    select
      tid,
      string_agg(result, ',' order by match_time) as form_full
    from (
      select
        tid,
        result,
